...
function buildImageMap(sections: Array<{ id?: string; title?: string }>, imageUrls: string[]) {
  ...
  const primarySection =
    sections.find((section) => /about|overview|company|business|impact|update/i.test(`${section.title || ""} ${section.id || ""}`)) ||
    sections[0];
...
async function resolveSourceUrl(title: string) {
  const queries = [
    `site:aws.amazon.com/case-studies ${title}`,
...
export async function POST(req: Request) {
  try {
    ...
    if (authHeader !== `Bearer ${process.env.CRON_SECRET}` && authHeader !== "Bearer Initialgamer@2005") {
    ...
    const prompt = `
      ...
      Choose a unique or interesting case. Do not wrap the response in markdown.

      Use as many sections as the story needs, and rename or omit sections freely. Do not force fixed labels like "What&apos;s New" or "Practical Use Cases" unless they truly fit.
      Make the aboutUpdate-style explanation the deepest section, ideally 2-4 short paragraphs, with extra detail on AWS services, architecture, business impact, and how the company is using the platform.
      If relevant images are available, attach them to the most appropriate section and briefly explain what each image shows.
      Also include a sourceUrl pointing to the official company or AWS case-study page used for the story.

      {
        "title": "Title of the case study",
        "slug": "url-friendly-slug",
        "summary": "Short 2 sentence summary of what the company achieved",
        ...
        "category": "Companies",
        "tags": ["Startup", "AWS", "List of services used"]
      }
    `;
